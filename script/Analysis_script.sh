...
# Update counts based on locate output, considering only matches at positions 1 to 8 for barcodes and 1 to 17 for probe motifs
while IFS=\$'\t' read -r seqID patternName pattern strand start end matched; do
    if [[ "\$start" -eq 1 && "\$end" -eq 8 ]]; then # use what is applicable
    if [[ "\$start" -eq 1 && "\$end" -eq 17 ]]; then # use what is applicable
